...
function apicalls(){
    ...
    if (typeof window.CONFIG === 'undefined' || !window.CONFIG.OPENWEATHER_API_KEY) {
        console.error('ERROR: API keys not configured. Please copy config.example.js to config.js and add your API keys.');
        alert('API keys not configured. Please see console for details.');
        return;
    }
    
    let urlstart = "https://api.openweathermap.org/data/2.5/onecall?lat=";
    let urlmitte = "&lon=";
    let urlende = "&exclude=minutely,hourly,alerts&lang=de&units=metric&appid=" + window.CONFIG.OPENWEATHER_API_KEY;
...
function geoAPIcall(lat, lon){
    ...
    const geoAPIurl = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + lat + ","  + lon + "&key=" + window.CONFIG.GOOGLE_MAPS_API_KEY;
